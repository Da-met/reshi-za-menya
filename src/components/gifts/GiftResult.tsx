// D:\МАЙО\JavaScript\ПРОЕКТЫ\РЕШИ ЗА МЕНЯ\reshi-za-menya\src\components\gifts\GiftResult.tsx
'use client';

import React, { useState } from 'react';
import { GiftResponse } from '@/types/gifts';
import { RotateCw, Check, Heart, Share2 } from 'lucide-react';
import { GiftProductCard } from '@/components/gifts';
...
function GiftResult({ gift, onSave, onGenerateAnother }: GiftResultProps) {
  ...
  return (
    <div className="space-y-6 md:space-y-8 mt-8">
      ...
      {/* Используем новый компонент GiftProductCard */}
      <GiftProductCard
        gift={gift.gift}
      ...
      <div className="bg-card rounded-2xl shadow-lg p-4 md:p-6">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={() => onGenerateAnother(gift.gift.title)}
...
export default React.memo(GiftResult);